...
# st.sidebar.subheader('Stijgende Aandelen:')
# for ticker in data:
#     percent_change, latest_close = data2['% Change']
#     if percent_change is not None:
#         if percent_change > 0:
#             st.sidebar.markdown(f"*{ticker}:* <span style='color:green'>{percent_change:.2f}% ↑</span>", unsafe_allow_html=True)
#         else:
#             st.sidebar.markdown(f"*{ticker}:* <span style='color:red'>{percent_change:.2f}% ↓</span>", unsafe_allow_html=True)
#     else:
#         st.sidebar.markdown(f"*{ticker}:* Data unavailable")
...
with tech_indicator:
    ...
    df = pd.DataFrame()
    desired_indicators = ["sma", "swma", "rsi", "bbands", "macd"]
...
with signals:
    st.write('Moet ik nou kopen of verkopen...')
    ...
    st.write('MACD signialen, met stoploss op 2,5% voor risico management')
